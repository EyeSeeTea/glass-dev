    ...
    const cmd = command({
        ...
        handler: async args => {
            try {
                ...
                consoleLogger.info(
                    `[${new Date().toISOString()}] >>> Target instance: ${envVars.url} (auth: ${
                        envVars.token ? "Personal Access Token" : `basic auth as ${envVars.auth?.username}`
                    })${args.calculate ? " | --calculate ENABLED (will create events)" : ""}`
                );
