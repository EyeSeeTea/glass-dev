...
export class VariableService {
    ...
    static getTrackedEntityValueForVariable(
        value,
        ...
        return hasValue &&
            ...
            trackedEntityAttributes[trackedEntityAttributeId].optionSetId
            ? OptionSetHelper.getName(
                  optionSets[trackedEntityAttributes[trackedEntityAttributeId].optionSetId].options,
                  value
              )
            : value;
    }
    ...
    getVariables({
        ...
        const variables = (programRuleVariables ?? []).reduce((accVariables, programVariable) => {
            ...
            if (!getterFn) {
                console.error(`Unknown programRuleVariableSourceType:${programVariable.programRuleVariableSourceType}`);
    ...
    buildVariable(value, type, { variablePrefix, allValues, variableEventDate, useNameForOptionSet = false }) {
        const processedAllValues = allValues
            ? allValues.map(alternateValue => this.onProcessValue(alternateValue, type))
            : null;
    ...
    getVariableForSelectedEntityAttributes(programVariable, sourceData) {
        // $FlowFixMe[incompatible-type] automated comment
        const trackedEntityAttributeId = programVariable.trackedEntityAttributeId;
    ...
    getVariableForCurrentEvent(programVariable, sourceData) {
        ...
        const value = VariableService.getDataElementValueForVariable(
            dataElementValue,
            dataElementId,
            programVariable.useNameForOptionSet,
    ...
    getVariableForNewestEventProgramStage(programVariable, sourceData) {
        ...
        if (!programStageId) {
            console.warn(
                `Variable id:${programVariable.id} name:${programVariable.displayName} does not have a programstage defined, despite that the variable has sourcetype${programVariable.programRuleVariableSourceType}`
            );
            return null;
        }
        ...
        const allValues = stageEvents
            .map(event =>
                VariableService.getDataElementValueForVariable(
                    event[dataElementId],
                    dataElementId,
                    programVariable.useNameForOptionSet,
                    sourceData.dataElements,
