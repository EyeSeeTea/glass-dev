export const d2Functions = (dateUtils, variableService, variablesHash, selectedOrgUnit, selectedUserRoles) => ({
    ...
    "d2:count": {
        ...
        dhisFunction: params => {
            ...
            if (variableObject) {
                ...
                console.warn(`could not find variable to count: ${variableName}`);
            }
            return count;
        },
    },
    "d2:countIfValue": {
        ...
        dhisFunction: params => {
            ...
            if (variableObject) {
                ...
                console.warn(`could not find variable to countifvalue: ${variableName}`);
            }
            return count;
        },
    },
    ...
    "d2:hasValue": {
        ...
        dhisFunction: params => {
            ...
            const variableObject = variablesHash[variableName];
            if (!variableObject) {
                console.warn(`could not find variable to check if has value: ${variableName}`);
            }
        ...
        dhisFunction: params => {
            ...
            if (variableObject) {
                if (variableObject.variableEventDate) {
                    ...
                    console.warn(`no last event date found for variable: ${variableName}`);
                }
            } else {
                console.warn(`could not find variable to check last event date: ${variableName}`);
            }
            return valueFound;
        },
    },
    "d2:addControlDigits": {
        ...
        dhisFunction: params => {
            ...
            if (baseDigits && baseDigits.length < 10) {
                ...
                if (firstDigit === 10) {
                    console.warn("First control digit became 10, replacing with 0");
                    firstDigit = 0;
                }
                if (secondDigit === 10) {
                    console.warn("Second control digit became 10, replacing with 0");
                    secondDigit = 0;
                }
            } else {
                console.warn(`Base number not well formed(${baseDigits.length} digits): ${baseNumber}`);
            }
